...
@section('title', 'Tecnología Sustentable')
        ...
        <div class="col-md-3 mb-4">
            <div class="card product-card h-100">
                ...
                <div class="card-body">
                    ...
                    <h6 class="text-success">{{ $product->formatted_price }}</h6>
                    <a href="/products/{{ $product->slug }}" class="btn btn-sm btn-outline-success w-100">Ver más</a>
                </div>
            </div>
        </div>
        @endforeach
    ...
    <div class="row">
        ...
        <div class="col-md-4 mb-4">
            <div class="card post-card h-100">
                ...
                @endif
                <div class="card-body">
                    <small class="text-muted">{{ $post->published_date->format('d/m/Y') }} | {{ $post->author }}</small>
                    <h5 class="card-title mt-2">{{ $post->title }}</h5>
                    <p class="card-text small">{{ Str::limit($post->excerpt, 100) }}</p>
                    <a href="/blog/{{ $post->slug }}" class="btn btn-sm btn-outline-success">Leer más</a>
                </div>
            </div>
        </div>
        @endforeach
