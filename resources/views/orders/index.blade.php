            ...
            <div class="col-md-6 mb-4">
                <div class="card h-100">
                    <div class="card-header bg-success text-white d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <span class="badge bg-light text-dark me-2">{{ ucfirst($order->status) }}</span>
                    ...
                    <div class="card-body">
                        ...
                        <p class="mb-2">
                            <strong>Total:</strong> <span class="text-success fs-5">${{ number_format($order->total, 2) }}</span>
                        </p>
                        @if($order->discount > 0)
                        <p class="mb-2 text-danger">
                            <strong>Descuento aplicado:</strong> -${{ number_format($order->discount, 2) }}
                        </p>
                        @endif
                        <p class="mb-2">
                        ...
                        <p class="mb-3">
                            <strong>Productos:</strong> {{ $order->items->sum('quantity') }} unidades
                        </p>
                        <div class="d-flex gap-2">
                            <button class="btn btn-outline-success btn-sm" onclick="showTracking({{ $order->id }})">
                                <i class="bi bi-truck"></i> Ver seguimiento
                            </button>
                            <button class="btn btn-success btn-sm" onclick="showOrderDetails({{ $order->id }})">
...
<div class="modal fade" id="deleteAllModal" tabindex="-1">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="modal-footer">
                <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Cancelar</button>
                <form id="deleteAllForm" method="POST" action="{{ route('orders.destroyAll') }}">
                    @csrf
                    @method('DELETE')
